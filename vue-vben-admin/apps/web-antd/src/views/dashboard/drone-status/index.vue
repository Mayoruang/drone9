...
// 导入MQTT API
import {
  sendConsoleMessageToDrone,
  sendMqttMessageToDrone,
  type MqttMessageRequest,
  type MqttMessageResponse
} from '#/api/mqtt';
...
const sendMqttMessage = async () => {
  ...
  try {
    // 根据消息类型选择不同的API
    let response: MqttMessageResponse;
    if (mqttForm.messageType === 'CONSOLE') {
      ...
      // 发送自定义MQTT消息
      const request: MqttMessageRequest = {
        topic: mqttForm.topic,
        message: mqttForm.message,
        messageType: mqttForm.messageType,
        qos: mqttForm.qos as 0 | 1 | 2,
        retained: mqttForm.retained,
      };
      response = await sendMqttMessageToDrone(selectedDrone.value.droneId, request);
    }
    ...
    >
      <Form layout="vertical">
        <Form.Item label="消息类型">
          <Select v-model:value="mqttForm.messageType" placeholder="选择消息类型">
            <Select.Option value="CONSOLE">控制台消息</Select.Option>
            <Select.Option value="COMMAND">指令消息</Select.Option>
            <Select.Option value="NOTIFICATION">通知消息</Select.Option>
            <Select.Option value="CUSTOM">自定义消息</Select.Option>
          </Select>
        </Form.Item>

        <Form.Item v-if="mqttForm.messageType !== 'CONSOLE'" label="MQTT主题">
          <Input v-model:value="mqttForm.topic" placeholder="输入MQTT主题" />
        </Form.Item>

        <Form.Item v-if="mqttForm.messageType === 'CONSOLE'" label="优先级">
          <Select v-model:value="mqttForm.priority" placeholder="选择优先级">
        ...
        <div class="grid grid-cols-2 gap-4">
          <Form.Item v-if="mqttForm.messageType === 'CONSOLE'" label="需要确认">
            <Switch v-model:checked="mqttForm.requireAck" />
          </Form.Item>

          <Form.Item v-if="mqttForm.messageType !== 'CONSOLE'" label="QoS级别">
            <Select v-model:value="mqttForm.qos" placeholder="选择QoS级别">
          ...
          <Form.Item v-if="mqttForm.messageType !== 'CONSOLE'" label="保留消息">
            <Switch v-model:checked="mqttForm.retained" />
          </Form.Item>
        </div>
      </Form>
    </Modal>
